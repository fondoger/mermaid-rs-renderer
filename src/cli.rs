...
use crate::render::{render_svg, try_write_png, write_output_svg};
use anyhow::Result;
...
pub fn run() -> Result<()> {
    ...
    if diagrams.len() == 1 {
        ...
        match args.output_format {
            ...
            OutputFormat::Png => {
                ...
                try_write_png(&svg, &output, &config.render, &config.theme)?;
            }
        }
    ...
    for (idx, diagram) in diagrams.iter().enumerate() {
        ...
        match args.output_format {
            ...
            OutputFormat::Png => {
                try_write_png(&svg, &outputs[idx], &config.render, &config.theme)?;
            }
        }
    }
